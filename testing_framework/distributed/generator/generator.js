...
const testingLatency = data.simulation.settings.testingLatency;
const testingThroughput = data.simulation.settings.testingThroughput;
...
function fetchRandomPublicationString() {
  let publication;
  let publicationType = probabilisticChoice([testingLatency, testingThroughput, 1-testingLatency-testingThroughput]);
...
function fetchInstructionSet() {
  ...
  if (testingThroughput != 0)
  {
    for (let k = 0; k < clientIDArray.length; k++) {
      ...
      subscription = subscription + clientID + " " + x + " " + y + " " + r + " " + channel;
      instructions = instructions + "\n" + subscription + "\n" + generateWaitInstruction(timeIntervalSubscription[0],timeIntervalSubscription[1]);
      let client1 = clients[clientIDArray[k]];
      let subscription1 = "subscribe ";
      let x1 = 500;
      let r1 = 100
      let y1 = 500
      let channel1 = "throughput1"
      let clientID1 = client.clientID
      subscription1 = subscription1 + clientID1 + " " + x1 + " " + y1 + " " + r1 + " " + channel1;
      instructions = instructions + "\n" + subscription1 + "\n" + generateWaitInstruction(timeIntervalSubscription[0],timeIntervalSubscription[1]);
    }
  }
  ...
  {
    if (testingThroughput!= 1 && testingLatency != 1)
    {
  ...
  while ((instructionInfo.numInstructionsToGenerate - instructionInfo.totalInstructionsGenerated) != 0) {
    ...
    switch (choice) {
      case 0:
        ...
        break;
      case 1:
        if (testingLatency == 0 && testingThroughput == 0)
...
function assignAddresses(info) {
  ...
  let static = {};
  static["master"] = { "hostname": "Lenovo", "user": info.hosts.computer.user, "static_IP_address": info.hosts.computer.ip, "time": timeInterval[0], "time1":timeOther[0]};
  static.clients = {};
  static.matchers = {};
